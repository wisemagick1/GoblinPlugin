...
public class GoblinGive implements CommandExecutor {
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("ggive")) {
                ...
                }else if(!sender.hasPermission("Goblin.ggive.others") && sender instanceof Player){
                        Player player = (Player) sender;
                        player.sendMessage(ChatColor.DARK_RED + "No permission!");
                    }
            }
            else if (args.length == 0 && sender instanceof ConsoleCommandSender != true && sender.hasPermission("Goblin.ggive")) {
                    Player player = (Player) sender;
                    Random rand = new Random(); //instance of random class
                    int upperbound = 13;
                    int i = rand.nextInt(upperbound);
